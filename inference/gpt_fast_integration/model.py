...
transformer_configs = {
    ...
    "llama-3.2-3b": dict(block_size=131072, n_layer=28, n_head=24, n_local_heads=8, dim=3072, intermediate_size=8192, vocab_size=128256, rope_base=500000,
        rope_scaling=dict(factor=32, low_freq_factor=1.0, high_freq_factor=4.0, original_max_position_embeddings=8192),
    ),
    ...
    # Qwen3 uses Q/K/V projection dimensions that do not necessarily match `dim`.
    # In particular, head_dim != dim/n_head for Qwen3-4B.
    "qwen3-4b": dict(block_size=40960, n_layer=36, n_head=32, n_local_heads=8, dim=2560, head_dim=128, intermediate_size=9728, vocab_size=151936, rope_base=1000000, use_qk_norm=True),
    "qwen3-8b": dict(block_size=40960, n_layer=36, n_head=32, n_local_heads=8, dim=4096, head_dim=128, intermediate_size=12288, vocab_size=151936, rope_base=1000000, use_qk_norm=True),
    "qwen3-14b": dict(block_size=40960, n_layer=40, n_head=40, n_local_heads=8, dim=5120, head_dim=128, intermediate_size=17408, vocab_size=151936, rope_base=1000000, use_qk_norm=True),
    "qwen3-32b": dict(block_size=40960, n_layer=64, n_head=64, n_local_heads=8, dim=5120, head_dim=128, intermediate_size=25600, vocab_size=151936, rope_base=1000000, use_qk_norm=True),
}
...
class Transformer(nn.Module):
    ...
    def setup_caches(self, max_batch_size, max_seq_length):
        ...
        elif hasattr(self.output, "fp_dtype"):
            dtype = self.output.fp_dtype
        else:
...
class Attention(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        q_size = config.n_head * config.head_dim
        kv_size = config.n_local_heads * config.head_dim
        total_head_dim = q_size + 2 * kv_size
        # key, query, value projections for all heads, but in a batch
        self.wqkv = nn.Linear(config.dim, total_head_dim, bias=False)
        self.wo = nn.Linear(q_size, config.dim, bias=False)

        # Optional Q/K normalization (used by some Qwen variants).
        # Applied per-head over head_dim.
        self.q_norm = RMSNorm(config.head_dim, eps=config.norm_eps) if config.use_qk_norm else None
        self.k_norm = RMSNorm(config.head_dim, eps=config.norm_eps) if config.use_qk_norm else None
        self.kv_cache = None
    ...
    def forward(self, x: Tensor, freqs_cis: Tensor, mask: BlockMask, input_pos: Optional[Tensor] = None) -> Tensor:
        ...
        q, k, v = self.wqkv(x).split([self.q_size, kv_size, kv_size], dim=-1)
        ...
        y = y.transpose(1, 2).contiguous().view(bsz, seqlen, self.q_size)
...
class FeedForward(nn.Module):
    def __init__(self, config: ModelArgs) -> None:
        ...
        # SwiGLU: fuse gate (w1) + up (w3) into a single matmul.
        # w1w3(x) -> [gate, up] where each is size intermediate_size.
        self.w1w3 = nn.Linear(config.dim, 2 * config.intermediate_size, bias=False)
