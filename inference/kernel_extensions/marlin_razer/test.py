...
def gen_fp4(m, n, groupsize=-1):
    ...
    layer = marlin_razer.Layer(256, 256, groupsize=groupsize)
...
class TestFP4(unittest.TestCase):
    ...
    def run_problem(self, m, n, k, thread_k, thread_n, groupsize=-1):
        ...
        marlin_razer.mul(A, B, C, s, workspace, thread_k, thread_n, -1)
    ...
    def test_auto_shapes_fp4(self):
        ...
        for m, n, k in shapes:
            ...
            marlin_razer.mul(A, B, C, s, workspace, 128, 128, -1)
            torch.cuda.synchronize()
            rel_err = torch.mean(torch.abs(C - C_ref)) / torch.mean(torch.abs(C_ref))
            ...
            marlin_razer.mul(A, B, C2, s, workspace)   # no thread_k/thread_n
            torch.cuda.synchronize()
